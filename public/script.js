...
let user = null;
let rPage = 1, resPage = 1;
let currentResources = [];
let currentReservations = [];
let currentDetailResource = null;
let editingResourceId = null;

const TYPES  = { salle_reunion:'Salle de réunion', terrain_sport:'Terrain de sport', coworking:'Coworking', coiffeur:'Coiffeur' };
const ICONS  = { salle_reunion:'🏢', terrain_sport:'⚽', coworking:'💻', coiffeur:'✂️' };
...
const empty = msg => `<div class="empty-state"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg><p>${msg}</p></div>`;
...
$('login-form').addEventListener('submit', e => {
  e.preventDefault();
  authSubmit('login-form', '/auth/login',
...
$('register-form').addEventListener('submit', e => {
  e.preventDefault();
  authSubmit('register-form', '/auth/register',
    () => ({ username: $('reg-username').value, email: $('reg-email').value, password: $('reg-password').value }),
    'Créer le compte', 'Création…');
});
...
async function launchApp() {
  ...
  if (!user) {
    try { const d = await api('/auth/me'); user = d.user || d.data; }
    catch { localStorage.removeItem('reserv_token'); token = null; showAuth(); return; }
  }
  ...
  const now = new Date();
  $('header-date').innerHTML = `${now.toLocaleDateString('fr-FR',{weekday:'long'})}<br>${now.toLocaleDateString('fr-FR',{day:'numeric',month:'long',year:'numeric'})}`;
  $('welcome-msg').textContent = `Bonjour, ${user.username || user.name || 'vous'}.`;
  loadDashboard();
}
...
document.querySelectorAll('.link-btn[data-goto]').forEach(btn => btn.addEventListener('click', () => {
...
async function loadDashboard() {
  try {
    const [rd, rv] = await Promise.all([api('/resources?limit=100'), api('/reservations/my?limit=100')]);
    const resources    = rd.data || rd.resources || [];
    const reservations = rv.data || rv.reservations || [];

    $('stat-resources').textContent       = resources.filter(r => r.available !== false).length;
    $('stat-my-reservations').textContent = reservations.length;
    $('stat-active').textContent          = reservations.filter(r => r.status === 'confirmed').length;

    $('recent-reservations').innerHTML = reservations.length === 0 ? empty('Aucune réservation') :
      reservations.slice(0,5).map(r => `
        <div class="reservation-item">
          <div class="res-dot ${r.status}"></div>
          <div class="res-info">
            <div class="res-title">${esc(r.title||'Sans titre')}</div>
            <div class="res-meta">${fmt(r.date)} · ${r.startTime}–${r.endTime}</div>
          </div>
          <span class="badge ${r.status}">${STATUS[r.status]||r.status}</span>
        </div>`).join('');

    $('popular-resources').innerHTML = resources.slice(0,5).map(r => `
      <div class="resource-mini-item">
        <div>
          <div style="font-size:.875rem;font-weight:500;color:var(--white)">${esc(r.name)}</div>
          <div style="font-size:.75rem;color:var(--gray-400);margin-top:2px">${TYPES[r.type]||r.type}</div>
        </div>
...
$('resource-search').addEventListener('input', e => {
  clearTimeout(debT);
  debT = setTimeout(() => { filters.search = e.target.value; rPage=1; loadResources(); }, 350);
});
$('resource-type-filter').addEventListener('change', e => { filters.type = e.target.value; rPage=1; loadResources(); });
$('resource-avail-filter').addEventListener('change', e => { filters.available = e.target.value; rPage=1; loadResources(); });
...
async function loadResources() {
  ...
  try {
    ...
    grid.innerHTML = list.length === 0
      ? `<div style="grid-column:1/-1">${empty('Aucune ressource trouvée')}</div>`
    ...
    grid.querySelectorAll('.resource-card').forEach((card, i) => {
      ...
      card.querySelector('.btn-book')?.addEventListener('click', ev => { ev.stopPropagation(); openBook(currentResources[i]); });
    });
    renderPages('resources-pagination', rPage, pages, 'resources');
  } catch(e) { grid.innerHTML = `<div style="grid-column:1/-1">${empty(e.message)}</div>`; }
}
...
function resCard(r, i) {
  ...
  return `<div class="resource-card" data-index="${i}">
    <div class="resource-card-top">
      ...
      <div class="resource-avail-pill ${ok?'available':'unavailable'}"></div>
    </div>
    <div class="resource-card-body">
      <div class="resource-card-name">${esc(r.name)}</div>
      <div class="resource-card-type">${TYPES[r.type]||r.type}</div>
      ${r.capacity?`<div class="resource-card-capacity"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>${r.capacity} personnes</div>`:''}
    </div>
    <div class="resource-card-footer">
      <span class="resource-price">${r.pricePerHour?`${r.pricePerHour}€/h`:'Gratuit'}</span>
      <button class="btn-book" ${ok?'':'disabled'}>${ok?'Réserver':'Indisponible'}</button>
    </div>
  </div>`;
}
  ...
  $('modal-resource-content').innerHTML = `
    <div class="resource-detail-header">
      ...
      <div>
        ...
        <div style="color:var(--gray-400);font-size:.875rem;margin-top:4px">${TYPES[r.type]||r.type}</div>
      </div>
    </div>
    <div class="resource-detail-attrs">
      <div class="attr-item"><div class="attr-label">Disponibilité</div><div class="attr-value" style="color:${ok?'var(--success)':'var(--gray-400)'}">${ok?'✓ Disponible':'✗ Indisponible'}</div></div>
      ${r.capacity?`<div class="attr-item"><div class="attr-label">Capacité</div><div class="attr-value">${r.capacity} personnes</div></div>`:''}
      ${r.pricePerHour!=null?`<div class="attr-item"><div class="attr-label">Tarif</div><div class="attr-value">${r.pricePerHour?`${r.pricePerHour}€/h`:'Gratuit'}</div></div>`:''}
      ${r.location?`<div class="attr-item"><div class="attr-label">Emplacement</div><div class="attr-value">${esc(r.location)}</div></div>`:''}
    </div>
    ${r.description?`<p style="color:var(--gray-400);font-size:.9rem;margin-bottom:24px;line-height:1.6">${esc(r.description)}</p>`:''}
    ${ok?`<button class="btn-primary" id="btn-reserver-maintenant"><span>Réserver maintenant</span><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></button>`:''}`;
  const btnR = $('btn-reserver-maintenant');
  if (btnR) btnR.addEventListener('click', () => { closeModal('modal-resource'); openBook(currentDetailResource); });
  $('modal-resource').classList.remove('hidden');
}

function openBook(r) {
  $('book-resource-id').value = r._id;
  $('modal-book').querySelector('.modal-title').textContent = `Réserver — ${r.name}`;
  $('book-form').reset();
...
$('check-avail-btn').addEventListener('click', async () => {
  ...
  const date = $('book-date').value, start = $('book-start').value, end = $('book-end').value;
  if (!date||!start||!end) { toast('Remplissez la date et les horaires', true); return; }
  try {
    const d = await post('/reservations/check-availability', { resourceId:$('book-resource-id').value, date, startTime:start, endTime:end });
    s.className  = `avail-status ${d.available?'available':'unavailable'}`;
    s.textContent = d.available ? '✓ Créneau disponible' : '✗ Créneau déjà réservé';
    s.classList.remove('hidden');
  } catch(e) { s.className='avail-status unavailable'; s.textContent=e.message; s.classList.remove('hidden'); }
});

$('book-form').addEventListener('submit', async e => {
  e.preventDefault();
  const errEl = $('book-error'), btn = e.target.querySelector('button[type=submit]');
  errEl.classList.add('hidden'); btn.disabled=true; btn.querySelector('span').textContent='Réservation…';
  try {
    await post('/reservations', {
      resourceId: $('book-resource-id').value, title: $('book-title').value,
      date: $('book-date').value, startTime: $('book-start').value,
      endTime: $('book-end').value, notes: $('book-notes').value,
    });
    closeModal('modal-book'); toast('Réservation créée !'); loadDashboard();
  } catch(e) { errEl.textContent=e.message; errEl.classList.remove('hidden'); }
  finally { btn.disabled=false; btn.querySelector('span').textContent='Confirmer'; }
});
...
async function loadMyReservations() {
  ...
  try {
    ...
    if (list.length === 0) { el.innerHTML = empty('Aucune réservation'); return; }
    el.innerHTML = `
      <div class="reservations-table-wrap"><table class="res-table">
        ...
        <tbody>${list.map((r,i)=>`
          <tr>
            <td style="font-weight:500;color:var(--white)">${esc(r.title||'—')}</td>
            <td>${esc(r.resource?.name||'—')}</td>
            <td style="font-family:var(--font-mono)">${fmt(r.date)}</td>
            <td style="font-family:var(--font-mono)">${r.startTime}–${r.endTime}</td>
            ...
            <td>${r.status!=='cancelled'
              ...
              :'<span style="color:var(--gray-600)">—</span>'}</td>
          </tr>`).join('')}
        </tbody>
      </table></div>`;
    el.querySelectorAll('.action-btn.danger').forEach(btn => {
      btn.addEventListener('click', () => cancelRes(currentReservations[parseInt(btn.dataset.index)]._id));
    });
...
async function cancelRes(id) {
  ...
  try { await patch(`/reservations/${id}/cancel`); toast('Réservation annulée'); loadMyReservations(); loadDashboard(); }
...
async function loadAdminResources() {
  ...
  try {
    ...
    const list = d.data || d.resources || [];
    if (list.length === 0) { el.innerHTML = empty('Aucune ressource'); return; }
    el.innerHTML = `
        ...
        <table class="res-table">
          ...
          <tbody>${list.map((r,i) => `
            <tr>
              ...
              <td><span class="badge ${r.available!==false?'confirmed':'cancelled'}">${r.available!==false?'Disponible':'Indisponible'}</span></td>
    ...
    el.querySelectorAll('.edit-res-btn').forEach(btn => {
      btn.addEventListener('click', () => openResourceForm(currentResources[parseInt(btn.dataset.index)]));
    });
    // Supprimer
    el.querySelectorAll('.del-res-btn').forEach(btn => {
      btn.addEventListener('click', () => deleteResource(currentResources[parseInt(btn.dataset.index)]));
    });
  } catch(e) { el.innerHTML = empty(e.message); }
}
      ...
      <div class="reservations-table-wrap">
        <table class="res-table">
          ...
          <tbody>${list.map(r=>`
            <tr>
              <td style="font-weight:500;color:var(--white)">${esc(r.title||'—')}</td>
              <td style="font-family:var(--font-mono);font-size:.8rem">${esc(r.user?.username||r.user?.email||'—')}</td>
              <td>${esc(r.resource?.name||'—')}</td>
              <td style="font-family:var(--font-mono)">${fmt(r.date)}</td>
              <td style="font-family:var(--font-mono)">${r.startTime}–${r.endTime}</td>
              <td><span class="badge ${r.status}">${STATUS[r.status]||r.status}</span></td>
...
function openResourceForm(r) {
  ...
  $('resource-form-title').textContent  = r ? 'Modifier la ressource' : 'Ajouter une ressource';
...
$('resource-form-cancel').addEventListener('click', () => closeModal('modal-resource-form'));
...
$('resource-form').addEventListener('submit', async e => {
  ...
  const body = {
    name:         $('rf-name').value,
    type:         $('rf-type').value,
    capacity:     $('rf-capacity').value ? parseInt($('rf-capacity').value) : undefined,
    pricePerHour: $('rf-price').value !== '' ? parseFloat($('rf-price').value) : undefined,
...
$('open-profile-btn').addEventListener('click', () => { loadProfile(); $('modal-profile').classList.remove('hidden'); });
...
$('cancel-delete-btn').addEventListener('click', () => closeModal('modal-confirm'));

$('confirm-delete-btn').addEventListener('click', async () => {
  if ($('confirm-delete-input').value !== 'SUPPRIMER') { toast('Tapez SUPPRIMER pour confirmer', true); return; }
  try {
    const res = await fetch('/api/auth/me', { method:'DELETE', headers:{ Authorization:`Bearer ${token}` } });
    if (!res.ok) { const d = await res.json().catch(()=>({})); throw new Error(d.message||'Route non disponible'); }
    closeModal('modal-confirm');
    toast('Compte supprimé. Au revoir !');
    setTimeout(() => { token=null; user=null; localStorage.removeItem('reserv_token'); showAuth(); }, 1500);
  } catch(e) { toast(e.message, true); }
});
...
function renderPages(containerId, cur, total, type) {
  ...
  let h = `<button class="page-btn" ${cur===1?'disabled':''} data-page="${cur-1}" data-type="${type}">‹</button>`;
  for (let i=1; i<=total; i++)
    h += `<button class="page-btn ${i===cur?'active':''}" data-page="${i}" data-type="${type}">${i}</button>`;
  h += `<button class="page-btn" ${cur===total?'disabled':''} data-page="${cur+1}" data-type="${type}">›</button>`;
  el.innerHTML = h;
  el.querySelectorAll('.page-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const page = parseInt(btn.dataset.page);
      if (btn.dataset.type === 'resources') { rPage=page; loadResources(); }
      else { resPage=page; loadMyReservations(); }
    });
  });
}
